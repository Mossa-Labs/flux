defmodule FluxWeb.CoreComponents do
  ...
  @doc """
  Renders a field label with an optional info tooltip.

  The tooltip appears on hover over a circular info icon next to the label text.
  ...
  def field_label(assigns) do
    ...
    <label class="label py-1">
      ...
      <span
        :if={@tooltip}
        id={String.replace(@text, ~r/[^a-zA-Z0-9]/, "-") <> "-tip"}
        phx-hook=".FieldTooltip"
        data-tip={@tooltip}
      >
        <.icon
          name="hero-information-circle"
          class="size-3.5 text-base-content/30 hover:text-base-content/60 cursor-help transition-colors"
        />
      </span>
      <script :type={Phoenix.LiveView.ColocatedHook} name=".FieldTooltip">
        export default {
          mounted() {
            ...
            this.el.addEventListener("mouseenter", () => {
              ...
              popup.className = "fixed z-[9999] max-w-56 rounded bg-neutral px-2 py-1.5 text-xs text-neutral-content shadow-lg whitespace-normal pointer-events-none";
          ...
          destroyed() {
            document.querySelectorAll(".fixed.z-\\[9999\\]").forEach(el => el.remove());
          }
        }
      </script>
    </label>
