defmodule FluxWeb.Components.UpgradePromptTest do
  ...
  test "default size renders a header and CTA link" do
    ...
    assert html =~ ~s(href="#{Flux.Marketing.pricing_url()}")
  end
  ...
  test "renders the Slack sink label" do
    html = render_component(&UpgradePrompt.upgrade_prompt/1, feature: :slack_sink)
    assert html =~ "Slack sink"
    assert html =~ "Flux Pro"
  end
